...
### Архитектура: мобильный vs десктоп

**Важно!** Чтобы избежать дублирования табов, следуйте этому паттерну:

#### Главные табы страницы (Треки/Альбомы/Артисты/Плейлисты)
- **Мобильный**: Табы в `PageHeader` (App.vue)
- **Десктоп**: Табы внутри компонента (LibraryView.vue, CollectionsView.vue)
- **Состояние**: Централизованно в `uiStore` (libraryTab, collectionsTab)

#### Вторичные переключатели (Моя библиотека/Общая)
- Всегда внутри компонента
- Видны и на мобильном, и на десктопе
- Используют локальный ref или localStorage
...
### Скрытие на мобильном

Для главных табов страницы используйте медиа-запрос:
...
#### LibraryView (главная библиотека)
- **Мобильный**: Табы в PageHeader
- **Десктоп**: Табы внутри компонента
- **Состояние**: `uiStore.libraryTab`

#### CollectionsView (коллекции)
- **Мобильный**: Табы "Альбомы/Плейлисты" в PageHeader
- **Десктоп**: Табы внутри компонента
- **Состояние**: `uiStore.collectionsTab`
- **Scope-switcher**: Всегда внутри (Моя библиотека/Общая)

#### ArtistsView (артисты)
- Без главных табов (это отдельная страница)
- Только scope-switcher (Моя библиотека/Общая)
