<template>
  <TransitionGroup name="task-widget-slide" tag="div" class="floating-tasks-container">
    <div
      v-for="item in tasksStore.activeMinimizedJobs"
      :key="item.job.id"
      class="floating-task-card"
      :class="[item.meta?.type || 'generic', item.job.status]"
      @click="handleRestore(item.job.id)"
      title="Нажмите, чтобы развернуть окно импорта"
    >
...
const subtextFor = (job) => {
  if (job.status === 'completed') return 'Импорт завершён'
...
.floating-task-card.exportify {
  ...
  box-shadow: 0 12px 32px rgba(0, 0, 0, 0.5), 0 0 24px rgba(29, 185, 84, 0.12);
}
...
.floating-task-card.import {
  ...
  box-shadow: 0 12px 32px rgba(0, 0, 0, 0.5), 0 0 24px rgba(255, 85, 0, 0.12);
}
