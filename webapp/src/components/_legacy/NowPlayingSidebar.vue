  ...
  <aside class="now-playing-sidebar">
    ...
    <div class="track-info">
      ...
      <button 
        ...
        @click="$emit('goToArtist', track.artist)"
      >
      ...
      <!-- Album (clickable) -->
      <button 
        v-if="track?.album?.name || track?.album_name" 
        class="info-link album-link"
        @click="$emit('goToAlbum', track.album?.id)"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 14.5c-2.49 0-4.5-2.01-4.5-4.5S9.51 7.5 12 7.5s4.5 2.01 4.5 4.5-2.01 4.5-4.5 4.5zm0-5.5c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1z"/>
        </svg>
        <span>{{ track.album?.name || track.album_name }}</span>
      </button>
    </div>
    ...
    <div class="source-info">
      ...
      <button 
        ...
        @click="$emit('goToUser', track.uploader)"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      >
        <svg v-if="track.forward_source.forward_from_type === 'channel'" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.17L4 17.17V4h16v12z"/>
        </svg>
        <svg v-else-if="track.forward_source.forward_from_type === 'bot'" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
const forwardSourceName = computed(() => {
  const fs = props.track?.forward_source
  if (!fs) return ''
...
.vinyl-label {
  ...
  height: 60px;
  border-radius: 50%;
  background: linear-gradient(135deg, var(--xm-primary) 0%, var(--xm-primary-dark) 100%);
...
/* Info Links (artist, album, uploader) */
.info-link {
  display: inline-flex;
  align-items: center;
  gap: 6px;
